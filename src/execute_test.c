...
int execute_halt(Instruction intruction, struct CurrentState state, int perform){
  return 4;
}

int execute_data_processing(Instruction intruction, struct CurrentState state, int perform) {
  return 2;
}

int execute_multiply(Instruction intruction, struct CurrentState state, int perform){
  return 3;
}

int execute_data_transfer(Instruction intruction, struct CurrentState state, int perform) {
  return 1;
}

int execute_branch(Instruction intruction, struct CurrentState state, int perform){
  return 0;
}

int execute(Instruction instruction, struct CurrentState state, int perform,enum instr_type type) {
  //instruction to be executed, machine state, boolean on whether to execute, instruction type
  int (*executions[5])(Instruction, struct CurrentState, int);
...
int main(void) {
  ...
  for(int i = 0; i < 50000; i++) {
    type_select = rand() % 5;
    if (execute(instruction,state,1,current_type[type_select]) != type_select) {
